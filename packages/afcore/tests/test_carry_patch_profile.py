...
# Locate the afcore package root relative to this test file.
# tests/ is at packages/afcore/tests/
# afcore source is at packages/afcore/afcore/
_AGENTFOX_PKG = Path(__file__).resolve().parent.parent / "afcore"
...
class TestCarryPatchModeEdgeCases:
    ...
    def test_unrecognised_mode_raises_key_error(self) -> None:
        ...
        try:
            from afcore.nightshift.carry_patch_monitor import (  # noqa: PLC0415
                resolve_carry_patch_mode,
            )
        except ImportError:
            pytest.fail(
                "resolve_carry_patch_mode wrapper must be importable from "
                "afcore.nightshift.carry_patch_monitor (03-REQ-5.E1)"
            )
