...
_JSONL_SINK_MODULE = _REPO_ROOT / "afcore" / "knowledge" / "jsonl_sink.py"
...
class TestJsonlSinkImports:
    ...
    def test_no_jsonl_sink_imports(self) -> None:
        ...
        agent_fox_dir = _REPO_ROOT / "afcore"
        py_files = list(agent_fox_dir.glob("**/*.py"))

        # Look for import statements referencing the deleted module
        _IMPORT_PREFIXES = (
            "from afcore.knowledge.jsonl_sink",
            "import afcore.knowledge.jsonl_sink",
        )
        import_matches = [
