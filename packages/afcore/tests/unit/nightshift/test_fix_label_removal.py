...
class TestEngineLabelFixedOnClose:
    ...
    async def test_fixed_label_assigned_on_supersession_close(self) -> None:
        ...
        with (
            ...
            patch("afcore.nightshift.engine.build_graph", return_value=[10, 12]),
            patch.object(engine, "_process_fix", AsyncMock()),
            patch("afcore.nightshift.engine.check_staleness", AsyncMock(return_value=MagicMock(obsolete_issues=[]))),
        ):
