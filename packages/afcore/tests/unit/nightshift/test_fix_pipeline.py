...
class TestPRBody:
    ...
    def test_pr_body_references_issue(self) -> None:
        ...
        from afcore.nightshift.fix_pipeline import build_pr_body
...
class TestSuccessfulFixHarvestsAndCloses:
    ...
    async def test_issue_not_closed_when_harvest_returns_no_changes(self) -> None:
        ...
        with (
            patch("afcore.workspace.harvest.harvest", AsyncMock(return_value=[])),
            patch("afcore.workspace.harvest.post_harvest_integrate", AsyncMock()),
        ):
...
class TestFixPipelineDbTelemetry:
    ...
    async def test_session_outcomes_written_for_each_session(self) -> None:
        ...
        with (
            patch("afcore.engine.state.record_session") as mock_record_session,
            patch("afcore.engine.state.update_run_totals") as mock_update_run_totals,
            patch("afcore.engine.state.create_run") as mock_create_run,
            patch("afcore.engine.state.complete_run") as mock_complete_run,
    ...
    async def test_session_outcome_has_run_id_and_archetype(self) -> None:
        ...
        from afcore.engine.state import SessionOutcomeRecord
        from afcore.nightshift.fix_pipeline import FixPipeline
        from afissues.protocol import IssueResult
        ...
        with (
            ...
            patch("afcore.engine.state.update_run_totals"),
            patch("afcore.engine.state.create_run"),
            patch("afcore.engine.state.complete_run"),
            patch.object(pipeline, "_harvest_and_push", AsyncMock(return_value="merged")),
        ):
    ...
    async def test_engine_passes_conn_to_fix_pipeline(self) -> None:
        ...
        original_fix_pipeline = __import__("afcore.nightshift.fix_pipeline", fromlist=["FixPipeline"]).FixPipeline
...
class TestExceptionSanitizationInFailureComment:
    ...
    async def test_failure_comment_omits_exception_message(self, caplog: pytest.LogCaptureFixture) -> None:
        ...
        with caplog.at_level(
            logging.WARNING,
            logger="afcore.nightshift.fix_pipeline",
        ):
            await pipeline.process_issue(issue, issue_body="Something is broken.")
