...
class TestMultiInstanceDispatch:
    ...
    async def test_multi_instance_creates_n_sessions(self) -> None:
        ...
        from afcore.session.convergence import converge_reviewer_pre, converge_verifier
...
class TestSkepticUnionDedup:
    ...
    def test_dedup_by_normalized_severity_description(self) -> None:
        from afcore.session.convergence import Finding, converge_reviewer_pre
        ...
        from afcore.session.convergence import normalize_finding

        unique = {normalize_finding(f) for f in merged}
        assert len(unique) == 4

    def test_empty_instances(self) -> None:
        from afcore.session.convergence import converge_reviewer_pre
...
class TestVerifierMajorityVote:
    ...
    def test_two_pass_one_fail(self) -> None:
        from afcore.session.convergence import converge_verifier
