...
class TestBackendProtocolIsinstance:
    ...
    def test_isinstance_claude_backend_is_backend(self) -> None:
        ...
        from afcore.session.backends import Backend, ClaudeBackend
    ...
    def test_backend_is_runtime_checkable(self) -> None:
        ...
        from afcore.session.backends import Backend
    ...
    def test_backend_importable_from_session_backends(self) -> None:
        """TS-02-1: Backend is importable from afcore.session.backends."""
        from afcore.session.backends import Backend
...
class TestBackendCloseIdempotent:
    ...
    async def test_close_idempotent_three_calls(self) -> None:
        ...
        from afcore.session.backends import ClaudeBackend
...
class TestProtocolImportIsolation:
    ...
    def test_protocol_import_does_not_load_sdk(self) -> None:
        ...
        import afcore.session.backends.protocol  # noqa: F401

        modules_after = set(sys.modules.keys())
        new_modules = modules_after - modules_before
        assert "claude_agent_sdk" not in new_modules
        assert "afcore.session.backends.claude" not in new_modules
...
class TestLazyImportIsolation:
    ...
    def test_importing_backends_does_not_load_sdk(self) -> None:
        """TS-02-8: Importing afcore.session.backends doesn't load SDK."""
        import subprocess
        ...
        result = subprocess.run(
            [
                ...
                (
                    "import sys; "
                    "import afcore.session.backends; "
...
class TestContainmentGlobScope:
    ...
    def test_glob_excludes_tests_directory(self) -> None:
        """TS-02-26: No file path under packages/afcore/tests/."""
        agent_fox_dir = os.path.join(
            os.path.dirname(__file__),
            "..",
            "..",
...
class TestCreateBackendGoogleMapping:
    ...
    def test_create_backend_google_maps_to_google_adk(self) -> None:
        ...
        with patch.dict(
            "sys.modules",
            {"afcore.session.backends.google_adk": mock_module},
        ):
            from afcore.session.backends import create_backend
...
class TestRunSessionUsesFactory:
    ...
    async def test_create_backend_called_with_config_value(self) -> None:
        ...
        with mock_patch(
            "afcore.session.session.create_backend",
...
class TestBackendsExports:
    ...
    def test_all_includes_backend_and_factory(self) -> None:
        ...
        import afcore.session.backends as backends_module
        from afcore.session.backends import Backend, create_backend  # noqa: F401
...
class TestNoHigherLevelReExports:
    ...
    def test_backend_not_in_afcore(self) -> None:
        """TS-02-22: from afcore import Backend raises ImportError."""
        with pytest.raises(ImportError):
            from afcore import Backend  # type: ignore[attr-defined]  # noqa: F401

    def test_backend_not_in_session(self) -> None:
        """TS-02-22: from afcore.session import Backend raises ImportError."""
        with pytest.raises(ImportError):
            from afcore.session import Backend  # type: ignore[attr-defined]  # noqa: F401

    def test_create_backend_not_in_afcore(self) -> None:
        """TS-02-22: from afcore import create_backend raises ImportError."""
        with pytest.raises(ImportError):
            from afcore import create_backend  # type: ignore[attr-defined]  # noqa: F401

    def test_create_backend_not_in_session(self) -> None:
        """TS-02-22: from afcore.session import create_backend raises ImportError."""
        with pytest.raises(ImportError):
            from afcore.session import create_backend  # type: ignore[attr-defined]  # noqa: F401
...
class TestInvalidTomlBackendCaughtEarly:
    ...
    def test_load_config_raises_before_create_backend(
        ...
        with mock_patch(
            "afcore.session.backends.create_backend",
        ) as mock_factory:
            with pytest.raises(ConfigError):
                load_config(toml_file)
    ...
    def test_subprocess_import_isolation(self) -> None:
        ...
        result = subprocess.run(
            [
                ...
                "-c",
                (
                    "import sys; import afcore.session.backends; "
                    "assert 'claude_agent_sdk' not in sys.modules, "
...
class TestContainmentMissingDirectory:
    ...
    def test_nonexistent_dir_raises_assertion(self) -> None:
        ...
        fake_dir = "/nonexistent/path/afcore/"
