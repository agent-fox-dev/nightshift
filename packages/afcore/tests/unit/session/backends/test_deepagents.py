...
class TestDeepAgentsBackendProtocol:
    ...
    def test_isinstance_backend(self) -> None:
        ...
        try:
            from afcore.session.backends.types import Backend
...
class TestSDKContainment:
    ...
    def test_deepagents_import_containment(self) -> None:
        ...
        # Navigate to the afcore source directory
        backends_dir = os.path.dirname(inspect.getfile(__import__("afcore.session.backends.types", fromlist=["types"])))
        afcore_dir = os.path.normpath(os.path.join(backends_dir, "..", ".."))

        allowed = os.path.normpath(os.path.join(afcore_dir, "session", "backends", "deepagents.py"))

        py_files = glob.glob(os.path.join(afcore_dir, "**", "*.py"), recursive=True)
        ...
        for py_file in py_files:
            ...
            if "import deepagents" in content or "from deepagents" in content:
                violations.append(os.path.relpath(py_file, afcore_dir))
...
class TestTypesModuleUnchanged:
    ...
    def test_deepagents_imports_canonical_types(self) -> None:
        ...
        import afcore.session.backends.deepagents as da_mod
        from afcore.session.backends import types
...
class TestPropertySDKContainment:
    ...
    def test_prop_sdk_containment(self) -> None:
        ...
        for root, _dirs, files in os.walk(afcore_dir):
            for fname in files:
                ...
                with open(full_path, encoding="utf-8") as f:
                    content = f.read()
                rel = os.path.relpath(full_path, afcore_dir)
...
class TestContainmentTestExtension:
    ...
    def test_containment_includes_deepagents(self) -> None:
        ...
        backends_dir = os.path.dirname(inspect.getfile(__import__("afcore.session.backends.types", fromlist=["types"])))
        afcore_dir = os.path.normpath(os.path.join(backends_dir, "..", ".."))
        allowed_file = os.path.normpath(os.path.join(afcore_dir, "session", "backends", "deepagents.py"))
...
class TestClaudeBackendBackwardCompat:
    ...
    def test_init_exports_unchanged(self) -> None:
        ...
        from afcore.session.backends import (
            AgentMessage,
            AssistantMessage,
            ClaudeBackend,
            PermissionCallback,
...
class TestCreateDeepAgentCall:
    ...
    async def test_create_deep_agent_called_with_params_and_tools(self) -> None:
        ...
        with patch(
            "afcore.session.backends.deepagents.create_deep_agent",
...
class TestFreshAgentOnRetry:
    ...
    async def test_no_duplicate_events_on_retry(self) -> None:
        ...
        """
        from afcore.session.backends.deepagents import DeepAgentsBackend
        from afcore.session.backends.types import AssistantMessage, ResultMessage
...
class TestBackoffBaseSharedConstant:
    ...
    def test_backoff_base_value(self) -> None:
        ...
        """
        from afcore.session.backends.claude import _BACKOFF_BASE as claude_backoff
        from afcore.session.backends.deepagents import _BACKOFF_BASE as da_backoff
    ...
    def test_backoff_base_not_duplicated(self) -> None:
        ...
        """
        import afcore.session.backends.claude as cl_mod
        import afcore.session.backends.deepagents as da_mod
