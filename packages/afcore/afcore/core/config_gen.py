...
def _get_bounds(model_class: type[BaseModel], field_name: str) -> str | None:
    ...
    from afcore.core.config import Clamped
...
def extract_schema(model: type[BaseModel], prefix: str = "") -> list[SectionSpec]:
    ...
    if not prefix:
        ...
        if has_nested:
            # Root model: each nested field becomes a section
...
def _render_schema_sections(schema: list[SectionSpec], lines: list[str]) -> str:
    """Append visible sections to *lines* and return the joined result with trailing newline."""
...
def generate_config_template(schema: list[SectionSpec]) -> str:
    ...
    return _render_schema_sections(schema, lines)
